 ...
 */
 ...
 (function (global, factory) {
	...
	function polygonContains(polygon, point) {
	  ...
	  var sum = new d3Array.exports.Adder();
	  ...
	  return function(sink) {
	    ...
	    var clip = {
	      ...
	      polygonEnd: function() {
	        ...
	        segments = d3Array.exports.merge(segments);
	  ...
	  return function(stream) {
	    ...
	    function polygonEnd() {
	      var startInside = polygonInside(),
	          ...
	          visible = (segments = d3Array.exports.merge(segments)).length;
	...
	function length(object) {
	  lengthSum$1 = new d3Array.exports.Adder();
	...
	function graticuleX(y0, y1, dy) {
	  var y = d3Array.exports.range(y0, y1 - epsilon, dy).concat(y1);
	  return function(x) { return y.map(function(y) { return [x, y]; }); };
	}

	function graticuleY(x0, x1, dx) {
	  var x = d3Array.exports.range(x0, x1 - epsilon, dx).concat(x1);
	...
	function graticule() {
	  ...
	  function lines() {
	    return d3Array.exports.range(ceil(X0 / DX) * DX, X1, DX).map(X)
	        .concat(d3Array.exports.range(ceil(Y0 / DY) * DY, Y1, DY).map(Y))
	        .concat(d3Array.exports.range(ceil(x0 / dx) * dx, x1, dx).filter(function(x) { return abs(x % DX) > epsilon; }).map(x))
	        .concat(d3Array.exports.range(ceil(y0 / dy) * dy, y1, dy).filter(function(y) { return abs(y % DY) > epsilon; }).map(y));
	  }
	...
	var areaSum = new d3Array.exports.Adder(),
	    areaRingSum = new d3Array.exports.Adder(),
	    x00$2,
	    y00$2,
	    x0$3,
	    y0$3;
	...
	var areaStream = {
	  ...
	  result: function() {
	    var area = areaSum / 2;
	    areaSum = new d3Array.exports.Adder();
	    return area;
	  }
	};
	...
	var lengthSum = new d3Array.exports.Adder(),
	    lengthRing,
	    x00,
	    y00,
	    x0,
	...
	var lengthStream = {
	  ...
	  result: function() {
	    ...
	    lengthSum = new d3Array.exports.Adder();
	    return length;
	  }
	};
	...
	var ramp$1 = scheme => d3Interpolate.exports.interpolateRgbBasis(scheme[scheme.length - 1]);
	...
	var d3ColorExports = requireD3Color();

	var interpolateCubehelixDefault = d3Interpolate.exports.interpolateCubehelixLong(d3ColorExports.cubehelix(300, 0.5, 0.0), d3ColorExports.cubehelix(-240, 0.5, 1.0));

	var warm = d3Interpolate.exports.interpolateCubehelixLong(d3ColorExports.cubehelix(-100, 0.75, 0.35), d3ColorExports.cubehelix(80, 1.50, 0.8));

	var cool = d3Interpolate.exports.interpolateCubehelixLong(d3ColorExports.cubehelix(260, 0.75, 0.35), d3ColorExports.cubehelix(80, 1.50, 0.8));

	var c$1 = d3ColorExports.cubehelix();
